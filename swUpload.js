...
class SwImageUpload extends HTMLElement {
    ...
    buildImgContainer(file, index) {
        const reader = new FileReader();
        const div = document.createElement('div');
        const img = document.createElement('img');
        img.title = file.name ?? '';
        img.alt = 'Image upload preview';
        reader.addEventListener('load', (e) => img.src = e.target.result, { once: true });
        ...
        div.addEventListener('keydown', (e) => {
            if (e.key === 'Delete' || e.key === 'Backspace') {
                ...
                if (this.#imgContainers)
                    this.#imgContainers[e.key === 'Backspace' ? (index - 1) : index]?.focus();
            }
        });
